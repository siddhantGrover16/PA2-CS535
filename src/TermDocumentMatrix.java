...
public class TermDocumentMatrix {
    ...
    public void indexTerms(File file, List<String> processedTerms) {
        ...
        termCounts = processedTerms.stream().collect(groupingBy(Function.identity(), summingInt(e -> 1)));//--//counting the terms
        documentToTermCounts.put(file.getName(), termCounts);
    }

    public List<String> getDocuments() {

        System.out.println(" size is "+ indexedDocuments.size());
               for(int i =0;i<indexedDocuments.size();i++){
           System.out.println(i + " "+indexedDocuments.get(i));
        }
        System.out.println(" ---------");
